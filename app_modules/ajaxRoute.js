...
module.exports = {
    ...
    fetchWeatherUsingMapCoOrds(dataType, res){
        ...
        if(dataType.lat){
            let latLng = dataType;
            fetchUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${latLng.lat}&lon=${latLng.lng}&units=metric&APPID=${process.env.SECRET_KEY}`;
        }

        if(dataType.cityName){
            fetchUrl = `https://api.openweathermap.org/data/2.5/weather?q=${dataType.cityName}&units=metric&APPID=${process.env.SECRET_KEY}`;
        }        
